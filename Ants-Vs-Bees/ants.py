...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current_place.entrance is not None:
            if not current_place.is_hive and self.lower_bound<=step<=self.upper_bound:
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while start:
            if isinstance(start.ant, ContainerAnt) and start.ant.ant_contained != None:
                start.ant.ant_contained.double()
            if start.ant:
                start.ant.double()
            start = start.exit
        # END Problem 12
